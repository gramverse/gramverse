...
export class FollowRepService {
    constructor(private followRepository: FollowRepository, private userRepService: UserRepService) {}
    ...
    getAllFollowers = async (userName: string) => {
        return (await this.followRepository.getAllFollowers(userName)).map(f => f.followerUserName);
    }
    ...
    update = async (followerUserName: string, followingUserName: string, follow: Partial<Follow>) => {
        return await this.followRepository.update(followerUserName, followingUserName, follow);
    }
}