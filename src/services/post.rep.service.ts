...
export class PostRepService {
    ...
    getPostsByUserName = async (userName: string, forCloseFriends: boolean, page: number, limit: number) => {
        const skip = (page-1) * limit;
        return await this.postRepository.getPostsByUserName(userName, forCloseFriends, skip, limit);
    }

    getPostCount = async (userName: string, forCloseFriends: boolean) => {
        return await this.postRepository.getPostCount(userName, forCloseFriends);
    }

    getExplorePosts = async (closeFriendsList: string[], followingsList: string[], page: number, limit: number) => {
        const skip = (page-1) * limit;
        return await this.postRepository.getExplorePosts(closeFriendsList, followingsList, skip, limit);
    }

    getExplorePostCount = async (closeFriendsList: string[], followingsList: string[]) => {
        return await this.postRepository.getExplorePostCount(closeFriendsList, followingsList);
    }
