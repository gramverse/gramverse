...
export class BlockRepository {
    ...
    block = async (followerUserName: string, followingUserName: string) => {
        const updateResult = await this.blocks.updateOne({followerUserName, followingUserName}, {isDeleted: true, followRequestState: FollowRequestState.NONE, isCloseFriend: false,isBlocked: true})
        return updateResult.acknowledged
    }

    unblock = async (followerUserName: string, followingUserName: string) => {
        const updateResult = await this.blocks.updateOne({followerUserName, followingUserName}, {isBlocked: false});
        return updateResult.acknowledged
    }
    ...
    getBlockList = async (followerUserName: string, skip: number, limit: number) => {
    ...
    getBlockListCount = async (followerUserName: string) => {
        return await this.blocks.countDocuments({followerUserName, isBlocked: true});
    }

    blockNonFollowing = async (followerUserName: string, followingUserName: string ) => {
       return !!(await this.blocks.create({followerUserName,followingUserName,isBlocked : true,followRequestState: FollowRequestState.NONE}))
    }
